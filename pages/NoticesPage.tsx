          ...
          {filtered.map(notice => {
            ...
            return (
              <div key={notice.id} className="bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-12 shadow-sm hover:shadow-2xl transition-all group flex flex-col md:flex-row gap-8 md:items-start relative overflow-hidden">
                ...
                <div className={`hidden md:flex w-20 h-20 rounded-3xl flex-col items-center justify-center shrink-0 ${theme.bg} ${theme.text} text-3xl group-hover:scale-110 transition-transform shadow-inner`}>
                ...
                <div className="flex-grow">
                  <div className="flex flex-wrap items-center gap-4 mb-6">
                    ...
                    <span className={`md:hidden px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${theme.bg} ${theme.text}`}>
                       {notice.category || 'Announcement'}
                    </span>
                    {notice.isImportant && (
                      <span className="px-3 py-1 bg-red-600 text-white rounded-full text-[9px] font-black animate-pulse">
                         CRITICAL
                      </span>
                    )}
                  </div>
