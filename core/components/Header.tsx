...
const Header: React.FC<HeaderProps> = ({ config, isAuthenticated = false, courses = [] }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-all duration-300 font-sans shadow-sm">
      {/* Light-Themed Admission Alert Bar */}
      {alert.enabled && (
        <div className="bg-slate-100 text-slate-900 py-2.5 px-4 h-9 flex items-center border-b border-slate-200">
          <div className="container mx-auto flex justify-between items-center text-[10px] font-black uppercase tracking-[0.1em]">
            <div className="flex items-center gap-2 md:gap-3">
               <span className="w-2 h-2 bg-[#ef4444] rounded-full shrink-0"></span>
               <div className="flex gap-2 whitespace-nowrap overflow-hidden">
                 <span className="text-emerald-600">{alert.text}</span>
                 <span className="hidden md:inline text-slate-900">{alert.subtext}</span>
               </div>
            </div>
            <Link to={alert.linkPath} className="text-emerald-600 hover:text-emerald-700 transition-all flex items-center gap-2 border-b-2 border-emerald-500/40 hover:border-emerald-600 pb-0.5 ml-4 shrink-0">
        ...
        <div className="container mx-auto px-4 h-full flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-7">
            ...
            <div className="flex items-center gap-4 ml-4">
              <Link to="/enroll" className="px-5 py-2.5 bg-emerald-600 text-slate-900 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-md active:scale-95">
                Enroll Now
              </Link>

              <Link to={isAuthenticated ? "/admin" : "/login"} className="px-5 py-2.5 bg-slate-100 text-slate-900 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all shadow-md flex items-center gap-2 active:scale-95 border border-slate-200">
                 <i className="fa-solid fa-gauge-high text-[10px] text-emerald-600"></i>
                 Admin
              </Link>
            </div>
          </nav>
        ...
        <div className="lg:hidden fixed inset-x-0 top-16 md:top-20 bg-white shadow-2xl z-[90] overflow-y-auto max-h-[calc(100vh-6rem)] border-t border-slate-100">
          <div className="flex flex-col p-6 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3 pt-4">
              <Link to="/enroll" className="bg-emerald-600 text-slate-900 font-black py-4 rounded-xl text-center uppercase tracking-widest text-[10px]" onClick={() => setIsMenuOpen(false)}>Enroll</Link>
              <Link to={isAuthenticated ? "/admin" : "/login"} className="bg-slate-100 text-slate-900 font-black py-4 rounded-xl text-center uppercase tracking-widest text-[10px] border border-slate-200" onClick={() => setIsMenuOpen(false)}>Dashboard</Link>
            </div>
          </div>
        </div>
      )}
