...
const NoticesPage: React.FC<NoticesPageProps> = ({ noticesState }) => {
  ...
  // Safety layer to prevent undefined errors
    ...
    <div className="min-h-screen bg-slate-50 overflow-x-hidden font-sans">
      <section className="bg-slate-900 pt-32 pb-24 text-white relative overflow-hidden">
        <div className="container mx-auto px-4 relative z-10 text-center max-w-4xl">
           ...
           <h1 className="text-4xl md:text-7xl font-black mb-8 tracking-tighter">{pageMeta.title || 'Campus Announcements'}</h1>
           ...
           <div className="flex flex-col md:flex-row gap-4 justify-center mt-10">
             <div className="relative w-full max-w-md mx-auto md:mx-0">
               ...
               <input 
                 ...
                 className="w-full pl-14 pr-6 py-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl text-white placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-emerald-500/30 transition-all font-medium text-base"
               />
             </div>
           </div>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {(['All', 'Urgent', 'Event', 'Holiday', 'New'] as const).map(cat => (
            <button
              ...
              className={`px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all shadow-xl active:scale-95 min-h-[44px] ${
                activeFilter === cat 
                  ...
                  <div className="flex-grow">
                    ...
                    {notice.link && (
                      <a href={notice.link} className="inline-flex items-center gap-3 px-8 py-4 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-emerald-600 transition-all shadow-xl">
                        View Details <i className="fa-solid fa-arrow-right-long" aria-hidden="true"></i>
                      </a>
                    )}
                  </div>
                </article>
