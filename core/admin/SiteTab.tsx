...
const SiteTab: React.FC<SiteTabProps> = ({ 
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (window.confirm("CRITICAL: Importing a database will overwrite all current site content. Continue?")) {
        onImport(content);
      }
    };
    ...
    <div className="space-y-12 animate-fade-in">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-3">
           ...
           <button onClick={handleImportClick} className="text-[10px] font-black bg-slate-700 hover:bg-slate-600 px-4 py-2.5 rounded-xl transition-all flex items-center gap-2">
             <i className="fa-solid fa-file-import"></i> IMPORT JSON
           </button>
           <button onClick={onExport} className="text-[10px] font-black bg-emerald-600 hover:bg-emerald-500 px-4 py-2.5 rounded-xl transition-all shadow-xl shadow-emerald-900/40 flex items-center gap-2">
             <i className="fa-solid fa-file-export"></i> EXPORT BACKUP
           </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {data.navigation.map((nav, idx) => (
            <div key={idx} className="bg-slate-800 p-5 rounded-2xl border border-slate-700 group relative">
              <button onClick={() => {if(window.confirm(`Remove ${nav.label}?`)) removeNavigation(idx)}} className="absolute -top-2 -right-2 w-8 h-8 bg-red-600 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center"><i className="fa-solid fa-trash-can text-xs"></i></button>
              <div className="grid grid-cols-1 gap-3">
                  <input value={nav.label} onChange={e => updateNavigation(idx, 'label', e.target.value)} className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-white" />
                  <input value={nav.path} onChange={e => updateNavigation(idx, 'path', e.target.value)} className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-emerald-500 font-mono" />
              </div>
            </div>
          ))}
        </div>
